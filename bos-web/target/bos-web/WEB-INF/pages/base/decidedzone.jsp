...
<head>
    ...
    <script type="text/javascript">
        ...
        function doAssociations() {
            ...
            if(rows.length != 1){
                $.messager.alert("提示信息","请选取一个定区！","warning");
            }else {
                // 选中了一个定区
                $("#customerWindow").window("open");
                // 打开下拉框之前要清理下拉框
                ...
                // 发送 Ajax 请求，给Customer查询关联的客户
                var url_1 = "customerAction_findListNotAssociation.action";
                $.post(url_1,function (data) {
                    ...
                    for (var i=0;i<data.length;i++){
                        ...
                        $("#noassociationSelect").append("<option value='"+id+"'>"+name+" "+telephone+"</option>");
                    }
                });

                var url_2 = "customerAction_findListHasAssociation.action";
                ...
                $.post(url_2,{"id":decidedzoneId},function (data) {
                    ...
                    for (var i=0;i<data.length;i++){
                        ...
                        var name = data[i].name;
                        var telephone = data[i].telephone;
                        $("#associationSelect").append("<option value='"+id+"'>"+name+" "+telephone+"</option>");
                    }
                });
            }
        }
        ...
        function doDblClickRow(index,data) {
            ...
            $('#association_customer').datagrid({
                ...
                rownumbers: true,
                striped: true,
                url: "customerAction_findListHasAssociation.action?id="+data.id,
                columns: [[{
                    field: 'id',
                    title: '客户编号',
                    width: 120,
...
<div modal="true" class="easyui-window" title="关联客户窗口" id="customerWindow" collapsible="false" closed="true" minimizable="false"
    ...
    <div style="overflow:auto;padding:5px;" border="false">
        <form id="customerForm"
              action="${pageContext.request.contextPath }/customerAction_assigncustomerstodecidedzone.action"
              method="post">
            <table class="table-edit" width="80%" align="center">
                <tr class="title">
